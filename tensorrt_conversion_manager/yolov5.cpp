...
ICudaEngine* build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name, const PrecisionMode &prec_mode) {
    ...
    if (prec_mode == PrecisionMode::USE_FP16) {
        config->setFlag(BuilderFlag::kFP16);
    } else if (prec_mode == PrecisionMode::USE_INT8) {
...
ICudaEngine* build_engine_p6(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name, const PrecisionMode &prec_mode) {
    INetworkDefinition* network = builder->createNetworkV2(0U);
    // Create input tensor of shape {3, INPUT_H, INPUT_W} with name INPUT_BLOB_NAME
    ITensor* data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{ 3, INPUT_H, INPUT_W });
    assert(data);
...
void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream, bool& is_p6, float& gd, float& gw, std::string& wts_name, const PrecisionMode &prec_mode) {
    // Create builder
    ...
    if (is_p6) {
        engine = build_engine_p6(maxBatchSize, builder, config, DataType::kFLOAT, gd, gw, wts_name, prec_mode);
    } else {
        engine = build_engine(maxBatchSize, builder, config, DataType::kFLOAT, gd, gw, wts_name, prec_mode);
    }
...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw, std::string& img_dir,
                int& BATCH_SIZE, float& NMS_THRESH, float& CONF_THRESH, PrecisionMode& prec_mode)
{
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        NMS_THRESH = atof(argv[6]);
        CONF_THRESH = atof(argv[7]);
        prec_mode = static_cast<PrecisionMode>(atoi(argv[8])); // 0 - USE_FP32, 1 - USE_FP16, 2 - USE_INT8
...
int main(int argc, char** argv) {
    ...
    float NMS_THRESH;
    float CONF_THRESH;
    PrecisionMode prec_mode;
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir, BATCH_SIZE, NMS_THRESH, CONF_THRESH, prec_mode)) {
    ...
    if (!wts_name.empty()) {
        ...
        APIToModel(BATCH_SIZE, &modelStream, is_p6, gd, gw, wts_name, prec_mode);
